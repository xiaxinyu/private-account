...
var groupview = $.extend({}, $.fn.datagrid.defaults.view, {
    ...
    onBeforeRender: function(target, rows){
        ...
        this.groups = groups;
        var that = this;
        setTimeout(function(){ that.bindEvents(target); },0);
...
$.extend(groupview, {
    ...
    insertRow: function(target, index, row){
        ...
        if (group){
            if (index == undefined || index == null){ index = state.data.rows.length; }
            if (index < group.startIndex){ index = group.startIndex; }
            else if (index > group.startIndex + group.rows.length){ index = group.startIndex + group.rows.length; }
            ...
            group = { value: row[opts.groupField], rows: [row], startIndex: state.data.rows.length };
